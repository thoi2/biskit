...
import storeCategories from '@/data/store_categories.json';
...
function makeTree(rows: Raw[]): Tree {
  const tree: Tree = {};
  rows.forEach(r => {
    const {상권업종대분류명: major, 상권업종중분류명: mid, 상권업종소분류명: sub} = r;
    tree[major] ??= {};
    tree[major][mid] ??= [];
...
export function BusinessFilter({selectedCategories, onFilterChange, setActiveTab}: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return Object.entries(businessCategories).filter(([major, minors]) =>
            ...
            <div className="space-y-2">
              {filtered.map(([major, minors]) => (
                  <Collapsible
                      ...
                      onOpenChange={() => toggle(expandedMajor, major, setExpandedMajor)}
                  >
                    ...
                    <CollapsibleContent className="ml-6 space-y-1">
                      {Object.entries(minors).map(([minor, subs]) => (
                          <Collapsible
                              ...
                              onOpenChange={() => toggle(expandedMinor, minor, setExpandedMinor)}
                          >
                            <CollapsibleTrigger asChild>
                            ...
                            <CollapsibleContent className="ml-6 space-y-1">
                              {subs.map(sub => (
                                  <Button
                                      key={sub}
                                      variant={
                                        selectedCategories.includes(sub) ? 'secondary' : 'ghost'
                                      }
