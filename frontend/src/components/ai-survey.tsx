...
const surveyQuestions: SurveyQuestion[] = [
  {
    ...
    type: "radio",
    options: ["처음 창업을 준비합니다", "1-2회 창업 경험이 있습니다", "3회 이상 창업 경험이 있습니다"],
  },
  {
    id: "budget",
    question: "초기 투자 가능한 예산 규모는 어느 정도인가요?",
    type: "radio",
    options: ["1천만원 미만", "1천만원 - 5천만원", "5천만원 - 1억원", "1억원 이상"],
  },
  {
    id: "industry_preference",
    question: "어떤 업종에 관심이 있으신가요?",
    type: "radio",
    options: ["음식점/카페", "소매업/유통", "서비스업", "IT/기술", "제조업", "잘 모르겠습니다"],
  },
  {
    id: "work_style",
    question: "선호하는 업무 스타일은 무엇인가요?",
    type: "radio",
    options: ["고객과 직접 소통하는 일", "혼자 집중해서 하는 일", "팀과 협업하는 일", "창의적인 일"],
  },
  {
    id: "location",
    question: "사업을 시작하고 싶은 지역은 어디인가요?",
    type: "radio",
    options: ["강남/서초구", "홍대/마포구", "명동/중구", "강북/성북구", "기타 서울 지역", "서울 외 지역"],
  },
  {
    id: "goals",
  ...
  const analyzeSurvey = async () => {
    ...
    try {
      ...
      if (user && analysisResults.recommendations.length > 0) {
        const interestsToSave = analysisResults.recommendations.map((result: SurveyResult) => ({
          user_id: user.userId,
      ...
      setResults([
        ...
        },
        {
          category: "온라인 쇼핑몰",
          confidence: 72,
          reasoning: "혼자 집중해서 하는 일을 선호하고 IT 분야에 관심이 있어 온라인 비즈니스에 적합한 성향을 보입니다.",
        },
        {
          category: "미용/뷰티",
          confidence: 68,
          reasoning: "고객과의 직접적인 소통을 중시하고 창의적인 일을 선호하는 특성이 미용업에 잘 맞습니다.",
        },
      ])
      setIsCompleted(true)
    } finally {
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground mb-4">당신의 답변을 바탕으로 AI가 추천하는 창업 분야입니다.</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">당신의 답변을 분석하여 최적의 창업 분야를 찾고 있습니다...</p>
          </div>
        </CardContent>
      </Card>
    )
