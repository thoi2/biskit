...
// 🚀 AI 테스트 함수 수정
  const testAI = async () => {
    ...
    try {
      // ✅ 올바른 백엔드 URL로 수정
      const response = await fetch('http://localhost:8080/api/v1/user/industry/ai-recommend', {
        method: 'POST',
      ...
      console.log('🔍 CORS 헤더:', response.headers.get('Access-Control-Allow-Origin'));

      if (response.ok) {
        const data = await response.json();
        console.log('✅ AI 추천 성공!', data);
        alert(`AI 추천 성공! ${data.data?.recommendations?.length || 0}개 업종 추천`);
      } else {
